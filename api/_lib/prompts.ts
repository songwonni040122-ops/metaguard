...
export function chatSystemPrompt(weakAxes: AxisKey[], badPicks: BadPick[], maxQuestions: number, turn: number): string {
  ...
  const modeLine = wrapUp
    ...
    : `- [이번 응답 규칙] 사용자의 답에 1문장 공감/반영한 뒤, 위 목록에서 "아직 묻지 않은 다음 선택" 하나를 골라 그 선택을 콕 집어 왜 그렇게 했는지 이유를 묻는 질문을 정확히 1개 던진다. 같은 선택을 반복해 파고들지 말고, 매 질문마다 반드시 서로 다른 선택으로 넘어간다. 목록의 선택을 모두 물었으면 남은 질문 수와 관계없이 더 질문하지 말고 곧바로 위 마무리 문장으로 끝낸다. (질문은 총 ${maxQuestions}개까지, 지금은 ${turn + 1}번째 질문)`;
  return [
    "너는 'MetaGuard'의 심리 진단 상담관이다. 대한민국 군 장병이 인지전(가짜뉴스·",
    '딥페이크·선동)에 대응하는 판단 습관을 스스로 돌아보도록 돕는다.',
    '',
    '[목표] 사용자가 진단에서 고른 "아쉬운 선택"들을 하나씩 서로 다르게 짚어, 각 선택마다 질문 1개씩 그 이유를',
    '  묻는다. (실수로 골랐거나 생각이 다를 수 있으므로, 이유를 들어 진단의 신빙성을 보정한다.)',
    '',
    '[이유를 물어야 할 아쉬운 선택들 — 위에서부터 하나씩, 질문마다 다른 선택]',
